...
contract YouWhoDapp is     
    ...
    // YouWhoInterface YouWhoInstance = YouWhoInterface(tokenMapping[bytes32("UHU")].tokenAddress);
    YouWho private uhuToken;
    AggregatorV3Interface internal priceFeed;

    event PaymentDone(address indexed user, address indexed provider, bytes32 booking, uint amount, bytes32 ticker, uint amountUhu, uint rating, uint fee);
    event DepositStake(address indexed user, uint uhuAmount, uint timeStart);
    event WithdrawStake(address indexed user, uint uhuAmount, uint depositTime, uint interest);
    event ReviewedUser(address indexed provider, bytes32 booking, uint rating, uint amountUhu);
    ...
    mapping(address => uint) public uhuBalanceOf;
    ...
    uint public uhuBack_; // amount of uhu returned as a % of USD value of payment, ie, input 10 for 10% back
    uint public minStakeTime_; // minimum amount of time to stake before being able to withdraw, in seconds
    uint public providerUhuRatio_; // the amount to divide the users uhu rewards by, eg if 10% of user rewards then set to 100/10 = 10, if 50% then 100/50 = 2
    bool[] public switches_; // switches for various functions
    

    constructor(address _uhu, YouWho uhu_, address _1) {

        uhuToken = uhu_;

        _setupRole(DEFAULT_ADMIN_ROLE, _1);
        _setupRole(ADMIN_ROLE, _1);
        _setupRole(TOKEN_ROLE, _1);
        _setupRole(STAKING_ROLE, _1);
        ...
        uhuBack_ = 1000; // 10% of each payment USD amount, uhu is minted to user, divide by 10000
        providerUhuRatio_ = 1000; // 100/ratio, eg, 100/1000 means provider will get 10% uhu that user gets, so if user get 300 provider will get 30
        ...
        bytes32 tickerUhu = bytes32("UHU");
        tokenMapping[tickerUhu] = Token(_uhu, address(0), 1 * (10**18), 18);
        tokenList.push(tickerUhu);
        ...
        bytes32 tickerUhusd = bytes32("UHUSD");
        tokenMapping[tickerUhusd] = Token(0xf572bb43CE1653A398950f31c5DCe6c47fA66012, address(0), 1 * (10**8), 8);
        tokenList.push(tickerUhusd);
        ...
        // bytes32 tickerUhusd = bytes32("UHUSD");
        // tokenMapping[tickerUhusd] = Token(0x7c875FCdf6728E16c51CFC542341110A9DD469cC, 0xa24de01df22b63d23Ebc1882a5E3d4ec0d907bFB, 1 * (10**8), 8);
        // tokenList.push(tickerUhusd);
    ...
    // function to change amount of UHU the user gets back for each payment
    function changeUhuBack(uint newValue) external {
        require(hasRole(ADMIN_ROLE, _msgSender()),"Error: You must have admin role to edit user UHU rewards.");
        uhuBack_ = newValue;
    }

    // function to change the amount of UHU the provider gets back when reviewing the user, if 10 then the provider gets 1/10 of UHU the user got
    function changeProviderUhuRatio(uint newValue) external {
        require(hasRole(ADMIN_ROLE, _msgSender()),"Error: You must have admin role to edit provider UHU rewards.");
        providerUhuRatio_ = newValue;
    }

    // function to change the minimum UHU staking time in seconds, 1 day = 86400 seconds
    ...
    // function to change yearly interest rate of uhu staking, newInterest should be an integer, eg input 10 for 10% interest pa
        ...
        if (switches_[0]) {
            ...
            if (uhuBack_ != 0) {
                ...
                if (tokenMapping[bytes32("ETH")].oracleAddress != address(0)) {
                    ...
                    amountUhu = msg.value * uint(price) / (10 ** uint(decimals)) * uhuBack_ / 10000; 
                    uhuToken.mint(msg.sender,amountUhu);

                } else {
                    amountUhu = msg.value * tokenMapping[bytes32("ETH")].manualTokenPrice / (10 ** tokenMapping[bytes32("ETH")].manualOracleDecimals) * uhuBack_ / 10000; 
                    uhuToken.mint(msg.sender,amountUhu);
                }

                claimableAmount[booking] = amountUhu * 100 / providerUhuRatio_;
                claimableAddress[booking] = provider;

            } 

            emit PaymentDone(msg.sender, provider, booking, msg.value, bytes32("ETH"), amountUhu, rating, fee_);
        ...
        if (switches_[1]) {
            ...
            if (uhuBack_ != 0) {
                ...
                if (tokenMapping[ticker].oracleAddress != address(0)) {
                    ...
                    amountUhu = amount * uint(price) / (10 ** uint(decimals)) * uhuBack_ / 10000;
                    uhuToken.mint(msg.sender,amountUhu);

                } else {
                    amountUhu = amount * tokenMapping[ticker].manualTokenPrice / (10 ** tokenMapping[ticker].manualOracleDecimals) * uhuBack_ / 10000;
                    uhuToken.mint(msg.sender,amountUhu);
                }
            ...
            emit PaymentDone(msg.sender, provider, booking, amount, ticker, amountUhu, rating, fee_);
    ...
    function depositStake(uint amount) public {
        ...
        if (switches_[2]) {
            ...
            require(amount >= (10**16), 'Error, deposit must be >= 0.01 UHU');

            uhuToken.transferFrom(msg.sender,address(this),amount);

            uhuBalanceOf[msg.sender] = amount;
    ...
    function myStakeInterest() public view returns (uint interest) {
        ...
        uint interestPerSecond = 3166802 * interest_ * uhuBalanceOf[msg.sender] / 1e18;
    ...
    function withdrawStake() public {
        ...
        //31668017 - interest(10% APY) per second for min. deposit amount (0.01 UHU), cuz:
        //1e15(10% of 0.01 UHU) / 31577600 (seconds in 365.25 days)

        //(uhuBalanceOf[msg.sender] / 1e16) - calc. how much higher interest will be (based on deposit), e.g.:
        //for min. deposit (0.01 UHU), (uhuBalanceOf[msg.sender] / 1e16) = 1 (the same, 31668017/s)
        //for deposit 0.02 UHU, (uhuBalanceOf[msg.sender] / 1e16) = 2 (doubled, (2*31668017)/s)
        uint interestPerSecond = 3166802 * interest_ * uhuBalanceOf[msg.sender] / 1e18;
        uint interestToPay = interestPerSecond * depositTime;

        uint userBalance = uhuBalanceOf[msg.sender]; //for event
        //reset depositer data
        depositStart[msg.sender] = 0;
        uhuBalanceOf[msg.sender] = 0;
        isDeposited[msg.sender] = false;

        uhuToken.mint(msg.sender, interestToPay); //interest to user
        uhuToken.transfer(msg.sender,userBalance); //send funds back to user
    ...
    function reviewClaim(bytes32 booking, uint rating) public {
        ...
        if (switches_[3]) {
            ...
            uhuToken.mint(msg.sender,claimableNow);
    ...
    function mintUhu(address _to, uint amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),"Error: You must have super admin role to mint UHU tokens.");
        uhuToken.mint(_to,amount);
    }
