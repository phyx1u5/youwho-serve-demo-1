        ...
        <Menu closeOnSelect={true}>
            ...
            <MenuButton>
                <Flex opacity={props.colorMode === "light" ? "1" : "0.85"} ml={["2", "3", "3"]} >
                    <Avatar bg={user ? "yellow.300" : "gray.300"} name={user ? user.attributes.username : ""} src={user ? (user.attributes.profile_photo ? user.attributes.profile_photo._url : gravatar !== "none" && gravatar !== "unchecked" ? `${gravatar}` : "") : ""} w={["42px", "48px", "48px"]} h={["42px", "48px", "48px"]} fontSize={["24px", "28px", "28px"]} ><AvatarBadge display={newEvent ? "block" : "none"} boxSize="0.8em" bg="red.400" borderWidth="3px" /></Avatar>
                </Flex>
            </MenuButton>
            <MenuList borderWidth="0" minWidth="250px" className="bgBlur" py={6}>
                ...
                {user ? (
                    <>
                        {newEvent &&
                            <MenuItem justifyContent="flex-end" mt="2" px={6} >
                                <NavLink to="/account/events" activeStyle={{ fontWeight: "bold", color: "#3182CE" }}>
                                    {newEvent ? <b>New Events</b> : "My Events"}<Icon as={newEvent ? BiI.BiCommentError : BiI.BiCommentDetail} color={newEvent ? "red.400" : "primary.400"} height="22px" width="22px" mb={1} ml={2} />
                                </NavLink>
                            </MenuItem>
                        }
                        <MenuItem justifyContent="flex-end" mt="2" px={6} >
                            <NavLink className="nav-link" to={`/account/profile`} fontWeight="bold" activeStyle={{ fontWeight: "bold", color: "#ff7e4c" }}>
                                {user ? user.attributes.username : ""}<Icon as={BiI.BiUserCircle} color="blue.500" height="22px" width="22px" mb={1} ml={2} />
                            </NavLink>
                        </MenuItem>
                        <MenuItem justifyContent="flex-end" mt="2" px={6} >
                            <Text as="button" className="nav-link" onClick={connectWallet}>
                                Connect Wallet<Icon as={CgI.CgPlug} color="green.300" height="22px" width="22px" mb={1} ml={2} />
                            </Text>
                        </MenuItem>
                        <Box display={[(history.location.pathname === "/" ? "block" : "none"), "block", "", ""]}>
                            <MenuItem mt="2" px={6} justifyContent="flex-end" >
                                <NavLink to="/newservice" activeStyle={{ fontWeight: "bold", color: "#3182CE" }}>
                                    Add New Service<Icon as={HiI.HiOutlinePencilAlt} color="secondary.200" height="22px" width="22px" mb={1} ml={2} />
                                </NavLink>
                            </MenuItem>
                            <MenuItem mt="2" px={6} justifyContent="flex-end" >
                                <NavLink to="/activity" activeStyle={{ fontWeight: "bold", color: "#ff7e4c" }}>
                                    Activity<Icon as={Io5.IoReaderOutline} color="primary.300" height="22px" width="22px" mb={1} ml={2} />
                                </NavLink>
                            </MenuItem>
                            <MenuItem mt="2" px={6} justifyContent="flex-end" >
                                <NavLink to="/chat" activeStyle={{ fontWeight: "bold", color: "#3182CE" }}>
                                    Chat<Icon as={Io5.IoChatboxEllipsesOutline} color="teal.300" height="22px" width="22px" mb={1} ml={2} />
                                </NavLink>
                            </MenuItem>
                            <MenuItem mt="2" px={6} justifyContent="flex-end" >
                                <NavLink to="/wallet" activeClassName="activeFooterLink" activeStyle={{ fontWeight: "bold", color: "#ff7e4c" }}>
                                    Wallet<Icon as={Io5.IoWalletOutline} color="yellow.400" height="22px" width="22px" mb={1} ml={2} />
                                </NavLink>
                            </MenuItem>
                        </Box>
                    ...
                    <>
                        <MenuItem mt="2" px={6} justifyContent="flex-end" >
                            <NavLink to="/activity/you/viewed" activeStyle={{ fontWeight: "bold", color: "#ff7e4c" }}>
                                Activity<Icon as={Io5.IoReaderOutline} color="primary.300" height="22px" width="22px" mb={1} ml={2} />
                            </NavLink>
                        </MenuItem>
                        <MenuItem justifyContent="flex-end" mt={2} px={6} onClick={onOpen}>
                            Sign Up / Login<Icon as={BiI.BiLogIn} color="green.500" height="22px" width="22px" mb={1} ml={2} />
                        </MenuItem>
                    </>
                )}
            </MenuList>
...
function ConfirmLogout({ logout, setYouWhoSwitch, history }) {
    ...
    return (
        <>
            <MenuItem as="button" justifyContent="flex-end" mt="2" px={6} onClick={onOpen} >
                Logout<Icon as={BiI.BiLogOut} color="red.300" height="22px" width="22px" mb={1} ml={2} />
            </MenuItem>
            <AlertDialog motionPreset="slideInBottom" leastDestructiveRef={cancelRef} onClose={onClose} isOpen={isOpen} isCentered >
                <AlertDialogOverlay />

                <AlertDialogContent m={3} borderRadius="lg" p={2} className="bgBlurModal" >
                    <AlertDialogHeader p={4}><Icon as={BiI.BiLogOut} w={6} h={6} mr={2} mb={1} />Confirm Logout</AlertDialogHeader>
