...
export default function TxHistory({ ethAddress }) {
    ...
    const reviewedUserBg = colorMode === "light" ? "yellow.300" : "yellow.500";
        ...
        <Stack>
            <Accordion defaultIndex={[0]} allowMultiple >
                        ...
                        <Stack spacing="3">
                            {paymentsReceived.length > 0 ?
                                paymentsReceived.map((x, i) =>
                                    <Box key={i + "alink"} as="a" target="_blank" href={`https://rinkeby.etherscan.io/tx/${x.attributes.transaction_hash}`} p={4} bg={paymentsReceivedBg} borderRadius="md">
                        ...
                        <Stack spacing="3">
                            {paymentsSent.length > 0 ?
                                paymentsSent.map((x, i) =>
                                    <Box key={i + "slink"} as="a" target="_blank" href={`https://rinkeby.etherscan.io/tx/${x.attributes.transaction_hash}`} p={4} bg={paymentsSentBg} borderRadius="md">
                                        ...
                                        <Text key={i + "sdetail"}>For booking <Text as="span" fontWeight="600">{web3.utils.hexToAscii(x.attributes.booking)}</Text> and was rewarded with <Text as="span" fontWeight="600">{Number(x.attributes.amountUhu / 10**18).toFixed(2)} UHU</Text> tokens as a Thank you from <Text as="span" fontWeight="600">YouWho.io</Text>!</Text>
                                    </Box>
                                )
                                :
                ...
                <AccordionItem borderBottom="0px">
                    <AccordionButton px={2} py={3} _hover={{ bg: "none" }}>
                        <Box flex="1" textAlign="left">
                            <Text color={grayText2}><Icon as={BsI.BsPersonCheck} color="yellow.500" mr={2} h={5} w={5} mb="4px" />{reviewedUser.length > 0 ? reviewedUser.length : "No"} Users Reviewed</Text>
                        </Box>
                        <AccordionIcon />
                    </AccordionButton>
                    <AccordionPanel px={0} pt={0} pb={3}>
                        <Stack spacing="3">
                            {reviewedUser.length > 0 ?
                                ...
                                <Box key={i + "rlink"} as={RouterLink} to={`/activity/who/bookings/${x.attributes.booking.id}`} p={4} bg={reviewedUserBg} borderRadius="md">
                                    <Text key={i + "rtitle"} mb={2} fontWeight="600"><Icon as={AiI.AiOutlineExclamationCircle} mr={2} h={6} w={6} mb="2px" />You Received {Number(x.attributes.providerUhu / 10 ** 18).toFixed(2)} UHU</Text>
