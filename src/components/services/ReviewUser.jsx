...
export default function ReviewUser({ Moralis, ethAddress, user, grayText, grayText2, history, bookedId, booking, web3, account, uhuDapp, siteIsLoading3, setSiteIsLoading3, siteIsLoading, setSiteIsLoading, reviewSuccess, review, setReviewSuccess, reviewRating, setReviewRating, reviewDescription, setReviewDescription, bookingUser, bookingUsername, claimableAmount }) {
    ...
    const reviewUser = async () => {
        ...
        if (uhuDapp !== 'undefined') {
            ...
            try {
                await uhuDapp.methods.reviewClaim(web3.utils.asciiToHex(bookedId), reviewRating).send({ from: ethAddress }).then(async () => {
            ...
            {(siteIsLoading && !bookingUser) ?
                ...
                <>
                    <Flex mb={3} mr={1} color={grayText2} align="center">
                        <Text fontWeight="400" fontSize="3xl" ml={1}  ><Icon as={BsI.BsListCheck} color="yellow.400" mb={1} mr={2} />Review User</Text>
                        <Spacer />
                    ...
                    {(reviewSuccess) ?
                        <>
                            <Stack spacing="6" p={2}>
                                <Center>
                                    <Text fontSize="xl" color="primary.400" fontWeight="300">Thank you for reviewing this booking. Your <b>UHU Token</b> rewards have been sent to your wallet.</Text>
                                </Center>
                            </Stack>
                        </>
                        :
                        <Stack spacing="6" p={2}>
                            ...
                            <Box pb={2} >
                                ...
                                <Text>You will receive approximately <Text as="span" color="secondary.300" fontWeight="600">{Number(claimableAmount / 10 ** 18).toFixed(4)} UHU</Text> Rewards after reviewing <Text as="span" color="primary.400" fontWeight="600">{bookingUsername}</Text></Text>
                            </Box>
