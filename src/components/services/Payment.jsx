...
export default function Payment({ history, grayText2, grayText, bookingId, ethAddress, setEthAddress, bookingUser, setBookingUser, booking, setBooking, bookingPublic, setBookingPublic, bookingApproval, setBookingApproval, ethAddresses, setEthAddresses, bookingStatus, setBookingStatus, rateAgreed, setRateAgreed, durationHours, setDurationHours, serviceLocation, setServiceLocation, paidAmountUsd, setPaidAmountUsd, dayTime, setDayTime }) {
    ...
    const [uhusd, setUhusd] = useState({});
    const [uhuDapp, setUhuDapp] = useState({});
    const [provider, setProvider] = useState("");
    const [providerName, setProviderName] = useState("");
    const [newPayment, setNewPayment] = useState("");
    const [myUhusdAllowance, setMyUhusdAllowance] = useState(0);
    ...
    console.log("myUhusdAllowance", myUhusdAllowance);
    ...
    let loadBlockchainData = async () => {
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            try {
                ...
                const uhusdC = new web3.eth.Contract(YouWhoUSD.abi, YouWhoUSD.networks[netId].address);
                setUhusd(uhusdC);

                const currUhusdAllowance = await uhusdC.methods.allowance(accounts[0], YouWhoDapp.networks[netId].address).call({ from: accounts[0] });
                setMyUhusdAllowance(currUhusdAllowance);
    ...
    const getTokenPrice = async () => {
        ...
        if (uhuDapp !== 'undefined') {
            try {
                await uhuDapp.methods.tokenMapping(web3.utils.asciiToHex(crypto)).call({ from: account }).then(async (result) => {
                    setCryptoDecimals(result["manualOracleDecimals"]);
                    const aggregatorV3InterfaceABI = [{ "inputs": [], "name": "decimals", "outputs": [{ "internalType": "uint8", "name": "", "type": "uint8" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "description", "outputs": [{ "internalType": "string", "name": "", "type": "string" }], "stateMutability": "view", "type": "function" }, { "inputs": [{ "internalType": "uint80", "name": "_roundId", "type": "uint80" }], "name": "getRoundData", "outputs": [{ "internalType": "uint80", "name": "roundId", "type": "uint80" }, { "internalType": "int256", "name": "answer", "type": "int256" }, { "internalType": "uint256", "name": "startedAt", "type": "uint256" }, { "internalType": "uint256", "name": "updatedAt", "type": "uint256" }, { "internalType": "uint80", "name": "answeredInRound", "type": "uint80" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "latestRoundData", "outputs": [{ "internalType": "uint80", "name": "roundId", "type": "uint80" }, { "internalType": "int256", "name": "answer", "type": "int256" }, { "internalType": "uint256", "name": "startedAt", "type": "uint256" }, { "internalType": "uint256", "name": "updatedAt", "type": "uint256" }, { "internalType": "uint80", "name": "answeredInRound", "type": "uint80" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "version", "outputs": [{ "internalType": "uint256", "name": "", "type": "uint256" }], "stateMutability": "view", "type": "function" }];
                    const addr = result["oracleAddress"];
                    const priceFeed = new web3.eth.Contract(aggregatorV3InterfaceABI, addr);
    ...
    const approveToken = async () => {
        if (uhusd !== 'undefined') {
            ...
            try {
                ...
                await uhusd.methods.approve(uhuDapp._address, web3.utils.toWei("9999999999999999999999999", "ether")).send({ from: account }).then(async () => {
                    const netId = await web3.eth.net.getId();
                    const currUhusdAllowance = await uhusd.methods.allowance(account, YouWhoDapp.networks[netId].address).call({ from: account });
                    setMyUhusdAllowance(currUhusdAllowance);

                    console.log("approve uhusd Success");
        ...
        if (uhuDapp !== 'undefined') {
            ...
            try {
                ...
                if (crypto === "ETH") {
                    ...
                    await uhuDapp.methods.makePaymentEth(ethAddress, web3.utils.asciiToHex(bookingId), reviewRating).send({ value: web3.utils.toWei(amount), from: account }).then(async (result) => {
                    ...
                    if (myUhusdAllowance > amount) {

                        await uhuDapp.methods.makePayment(ethAddress, web3.utils.asciiToHex(bookingId), web3.utils.toWei(amount), web3.utils.asciiToHex(crypto), reviewRating).send({ from: account }).then(async (result) => {
                            await Moralis.Cloud.run("setPaymentAmount", { paymentAmount, bookingPublic: bookingPublic.id, crypto, paymentCrypto: paymentAmount / cryptoPrice, priceAtTime: cryptoPrice });
                            setNewPayment(result);
                            setPaymentSuccess(true);
                            setSiteIsLoading(false);
        ...
        return (
            <>
                <Flex mb={3} mr={1} color={grayText2} align="center">
                    <Text fontWeight="400" fontSize="3xl" ml={1}  ><Icon as={RiI.RiHandCoinLine} color="green.300" mb={1} mr={2} />Pay Now</Text>
                    <Spacer />
...
function PayNow({ siteIsLoading, paymentSuccess, grayText, history, bookingId, payNow, booking, bookingPublic, newPayment, provider, bookingStatus, ethAddress, providerName, myUhusdAllowance, paymentAmount, cryptoPrice, crypto, account, approveToken }) {
        ...
        <>
            ...
            <Modal isOpen={isOpen} size="lg" onClose={paymentSuccess ? () => { onClose(); history.push(`/activity/you/booked/${bookingId}`); window.location.reload(); } : () => { onClose(); }} >
                ...
                <ModalContent minHeight="200px" borderRadius="xl" mx={3} mt="10%" bg={colorMode === "light" ? "white" : "gray.900"} overflow="hidden">
                    {paymentSuccess ? (
                        <>
                            <ModalHeader py={6} px={["4", "6", "", ""]} fontWeight="400"><Icon as={HiI.HiOutlineBadgeCheck} color="secondary.300" mb={1} mr={1} h="24px" w="24px" />Payment Success!</ModalHeader>
                    ...
                    ) : ((myUhusdAllowance > (paymentAmount / cryptoPrice) || crypto === "ETH") ?
                        <>
                            ...
                            <ModalBody px={["4", "6", "", ""]} pt={4}>
                                {siteIsLoading ?
                                    ...
                                    <Stack spacing="3">
                                        ...
                                        <Box pb={3}>
                                            <Text fontWeight="500" color={grayText}>Approximate UHU Token rewards received</Text>
                                            <Flex pt="2">
                                                <Box w="100%">
                                                    <Text ><b>{paymentAmount / 10} UHU</b></Text>
                                                </Box>
                                            </Flex>
                                        </Box>
                                ...
                                {siteIsLoading ?
                                    ...
                                    <Stack spacing="3">
                                        ...
                                        <Box pb={3} >
                                            <Flex pt="2">
                                                <Box w="100%">
                                                    <Text color={grayText} >As this is your first time paying with <Text as="span" color="primary.500" fontWeight="600">UHUSD</Text> at <Text as="span" color="secondary.300" fontWeight="600">youwho.io</Text>, you must first allow <Text as="span" color="secondary.300" fontWeight="600">youwho.io</Text> to make payments using your <Text as="span" color="primary.500" fontWeight="600">UHUSD</Text> tokens. Please click on the <Text as="span" color="primary.500" fontWeight="600">Approve</Text> button below.</Text>
                                                </Box>
                                            </Flex>
                                        </Box>
