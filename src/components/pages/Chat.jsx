...
export default function Chat() {
            ...
            onLiveUpdate: (youChats) => {
                ...
                if (!toast.isActive(toastIdyou)) {
                    toast({
                        ...
                        isClosable: true,
                        render: () => (
                            <Box as="button" textAlign="left" onClick={() => history.push("/chat/you")} w={["94vw", "512px", "", ""]} position="fixed" top="0" left="50%" p={4} pb={5} ml={["-47vw", "-251px", "", ""]} color="yellow.600" bg="yellow.200" borderBottomRadius="lg">
    ...
    const bgGradientLight = "radial-gradient(circle at 30vw 90vh, rgba(255,48,0,0.07) 0%, rgba(249,249,249,0.07) 50%, rgba(249,249,249,0.0) 100%)";

    // const bgGradientLight = "radial-gradient(circle at 30vw 90vh, rgba(255,48,0,0.07) 0%, rgba(249,249,249,0.07) 50%, rgba(249,249,249,0.0) 100%)";

    const bgGradientDark = "radial-gradient(circle at 80vw -30vh, rgba(245, 215, 147, 0.25) 0%, rgba(13,110,253,0.15) 35%, rgba(19,19,19,0.5) 100%)";
                    ...
                    <Container minHeight={["91vh", "400px", "", ""]} maxW="lg" borderRadius={["0", "xl", "xl", "xl"]} px={[4, 6, "", ""]} pb={[4, 6, "", ""]} pt={[1, 5, "", ""]} mb={["60px", "0", "", ""]} >
                        <Stack pb={2} spacing="0" >
                            <Flex mb={3} mr={1} color={grayText2}>
                                <Text fontWeight="400" fontSize="3xl" ml={1} color={grayText2}><Icon as={Io5.IoChatboxEllipsesOutline} color="teal.300" mb={1} mr={2} />My Chats</Text>
                                <Spacer />
                                        ...
                                        userChats.map((x, i) =>
                                            ...
                                            <Flex key={x.attributes.booking.id + "flex"} maxHeight={["19vw", "80px", ""]} role="button" onClick={() => { setSelectedUserChat(x); history.push(`/chat/you/${x.attributes.booking.id}`); }} bg={x.attributes.userRead ? "gray.5" : "rgba(255, 163, 126, 0.2)"} borderRadius="md" overflow="hidden" align="center">
                                                ...
                                                <Box p={2}>
                                                    ...
                                                    <Text key={i + x.attributes.booking.attributes.bookingPublic.attributes.approval} textTransform="uppercase" fontSize={["xs", "sm", "", ""]} color={x.attributes.booking.attributes.bookingPublic.attributes.approval === "pending" ? "yellow.400" : x.attributes.booking.attributes.bookingPublic.attributes.approval === "approved" ? "primary.600" : x.attributes.booking.attributes.bookingPublic.attributes.approval === "paid" ? "green.300" : "crimson"} >{x.attributes.booking.attributes.bookingPublic.attributes.approval}</Text>
                                                </Box>
                                                <Spacer />
                                                <Center p={2}>
                                ...
                                <Stack spacing="3">
                                    {providerChats.length > 0 ?
                                        providerChats.map((x, i) =>
                                            ...
                                            <Flex key={x.attributes.booking.id + "flex"} role="button" maxHeight={["19vw", "80px", ""]} onClick={() => { setSelectedProviderChat(x); history.push(`/chat/who/${x.attributes.booking.id}`); }} bg={!x.attributes.providerRead ? "gray.5" : "rgba(255, 212, 0, 0.2)"} borderRadius="md" overflow="hidden" align="center">
                                                <Box id={"flex1" + i} key={"flex1" + i} minWidth={["19vw", "80px", ""]} minHeight={["19vw", "80px", ""]} >
                                                    {x.attributes.service.attributes.service_img1 ? (
                                                        <Image id={"img1" + i} key={"img1" + i} w={["19vw", "80px", ""]} h={["19vw", "80px", ""]} objectFit="cover" src={x.attributes.service.attributes.service_img1._url} fallback={<Center w={["19vw", "80px", ""]} h={["19vw", "80px", ""]}><Spinner /></Center>} borderLeftRadius="md" />
                                                    ) : (
